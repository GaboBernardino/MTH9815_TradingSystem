...
public:
  ...
  // Set the quantity
  void SetQuantity(const long& newQuantity);
...
public:
  ...
  virtual const PV01< BucketedSector<T> >& GetBucketedRisk(std::string& sectorName) const = 0;

  // Update the bucketed sector risk
  virtual void UpdateBucketedRisk(std::string& sector) = 0;
};

template<typename T>
PV01<T>::PV01(const T &_product, double _pv01, long _quantity) :
  product(_product), pv01(_pv01)
{
  // pv01 = _pv01;
  quantity = _quantity;
}
...
template <typename T>
void PV01<T>::SetQuantity(const long& newQuantity)
{

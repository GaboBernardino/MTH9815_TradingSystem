...
public:
  ...
  //Set the price quoted on the inquiry
  void SetPrice(const double& newPrice);

  //Set the state
  void SetState(const InquiryState& newState);
...
template<typename T>
void Inquiry<T>::SetPrice(const double& newPrice)
{
  price = newPrice;
}

template<typename T>
void Inquiry<T>::SetState(const InquiryState& newState)
{
  state = newState;
}
